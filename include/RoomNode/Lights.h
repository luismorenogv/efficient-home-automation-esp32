...
private:
    ...
    const uint16_t VERIFY_DELAY_MS        = 400;  // ms delay after command
    const float    DARK_THRESHOLD         = 50.0; // Lux below which it's considered dark
    const float    BRIGHT_THRESHOLD       = 150.0; // Lux above which it's considered bright
